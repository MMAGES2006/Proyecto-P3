...
void Grid::generateMap()
{
  ...
  for (int leftMainPath = mainCombatRooms + 1; leftMainPath > 0; leftMainPath--)
  {
    for (int i = 0; i < 4; i++)
    {
      ...
      {
        if (this->miniMap[i][j])
          fakeMap[i][j] = this->miniMap[i][j]->type;
        else
          fakeMap[i][j] = VOID;
      }
    }
    if (this->enoughLenght(x, y - 1, leftMainPath, fakeMap))
      neighbors.push_back({x, y - 1});
    if (this->enoughLenght(x - 1, y, leftMainPath, fakeMap))
...
void Grid::toggle(int x, int y)
{
  ...
  if (this->map[indexX][indexY] == 2)
    this->map[indexX][indexY] = -4;
}
  ...
  if (type == SPECIAL)
  {
    for (int i = 0; i < 4; i++)
    {
      for (int j = 0; j < 4; j++)
      {
        if (this->miniMap[i][j])
        {
          if (this->potentialRoom(i, j - 1))
            evaluationMap[i][j - 1] = 1;
          if (this->potentialRoom(i - 1, j))
            evaluationMap[i - 1][j] = 1;
          if (this->potentialRoom(i + 1, j))
            evaluationMap[i + 1][j] = 1;
          if (this->potentialRoom(i, j + 1))
            evaluationMap[i][j + 1] = 1;
...
void Grid::createRoom(int x, int y, RoomType type)
{
  ...
  for (int i = 0; i < cellWidth; i++)
  {
    for (int j = 0; j < cellHeight; j++)
      this->map[i + positionX][j + positionY] = room->design[i][j];
  }
}
