...
void Player::control(RenderWindow &window, float time)
{
  ...
  {
    direction = UP;
    if (!this->collisionMap(this->x, this->y - displacement, UP))
      sprite.move(0, -displacement);
  }
  if (Keyboard::isKeyPressed(Keyboard::A))
  {
    direction = LEFT;
    if (!this->collisionMap(this->x - displacement, this->y, LEFT))
      sprite.move(-displacement, 0);
  }
  if (Keyboard::isKeyPressed(Keyboard::S))
  {
    direction = DOWN;
    if (!this->collisionMap(this->x, this->y + displacement, DOWN))
      sprite.move(0, displacement);
  }
  if (Keyboard::isKeyPressed(Keyboard::D))
  {
    direction = RIGHT;
    if (!this->collisionMap(this->x + displacement, this->y, RIGHT))
      sprite.move(displacement, 0);
  }
  ...
  if (Mouse::isButtonPressed(Mouse::Left))
  {
    ...
    {
      if (*(this->playing))
        this->gun->fire(Vector2f(Mouse::getPosition(window)), Vector2f(900, 500));
      else
        this->gun->fire(Vector2f(Mouse::getPosition(window)), Vector2f(this->x, this->y));
    }
    else
      this->coldWeapon->hit(this->grid->activeRoom->enemies, NULL, direction);
  }
